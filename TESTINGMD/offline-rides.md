...
- [ ] Start **Live** tracking on the offline ride → GPS tracking still works (route line only, no street basemap), with any control points shown on the map.
- [ ] While offline, the **Control points** card is present and fully usable — you can add, edit and delete points with no connection, since they're stored on the device. See [control-points.md](control-points.md#offline).
